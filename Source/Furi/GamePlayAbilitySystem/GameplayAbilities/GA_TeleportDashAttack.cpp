...
void UGA_TeleportDashAttack::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
    ...
    // 타격 이벤트 리스너
    UAbilityTask_WaitGameplayEvent* WaitHitEventTask = UAbilityTask_WaitGameplayEvent::WaitGameplayEvent(this, HitEventTag);
    if (WaitHitEventTask)
    {
...
{
    AActor* MyAvatar = GetAvatarActorFromActorInfo();
    if (!MyAvatar || !LockedTarget) return;
        ...
        for (const FOverlapResult& Result : OverlapResults)
        {
            ...
            if (Result.GetActor() == LockedTarget)
            {
                ...
                if (TargetChar && DamageGEClass)
                {
                    ...
                    DamageInfo.DamageResponse = (CurrentStrikeCount == MaxStrikeCount) ? EFuriDamageResponse::KnockBack : EFuriDamageResponse::Stagger;
...
void UGA_TeleportDashAttack::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
